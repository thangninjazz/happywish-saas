...
export default async function AdminDashboardPage() {
  try {
    ...
    if (userError) throw new Error(`User Stats Error: ${userError.message}`);

    const { count: templateCount, error: templateError } = await supabase.from('templates').select('*', { count: 'exact', head: true });
    if (templateError) throw new Error(`Template Stats Error: ${templateError.message}`);

    const { count: wishCount, error: wishError } = await supabase.from('wishes').select('*', { count: 'exact', head: true });
    if (wishError) throw new Error(`Wish Stats Error: ${wishError.message}`);

    const { count: orderCount, error: orderError } = await supabase.from('orders').select('*', { count: 'exact', head: true });
    if (orderError) throw new Error(`Order Stats Error: ${orderError.message}`);
    ...
    return (
      <div className="p-8 text-center bg-destructive/10 text-destructive rounded-3xl border border-destructive/20">
        <h2 className="text-xl font-bold mb-2">Lỗi Server Nghiêm Trọng</h2>
        <p className="font-mono text-sm break-all">{error.message}</p>
        <p className="text-xs mt-4">Vui lòng chụp màn hình này để báo cáo lỗi.</p>
      </div>
    );
  }
}
